...
use crate::helpers::getters::{get_custom_emoji, get_message_url};
use crate::helpers::permission;
use serenity::client::Context;
use serenity::framework::standard::macros::command;
use serenity::framework::standard::{Args, CommandResult, Delimiter};
use serenity::model::channel::{Message, ReactionType};
use serenity::model::id::{ChannelId, EmojiId, MessageId, RoleId};
use serenity::model::misc::Mentionable;
use serenity::prelude::SerenityError;
use serenity::static_assertions::_core::time::Duration;
use serenity::utils::{parse_channel, parse_emoji};

// THE REACTION ROLE SYSTEM IS CLOSE TO A 1:1 COPY FROM
// https://github.com/bdashore3/RoyalGuard/blob/rust/src/commands/reaction_roles.rs
...
async fn new(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    if !permission::check_administrator(ctx, msg, None).await? {
        return Ok(());
    }
    ...
    let channel_id = match parse_channel(&channel_test) {
        ...
        None => {
            msg.channel_id.say(ctx, "Invalid Channel").await?;
    ...
    let msg_id = match args.single::<u64>() {
        Ok(msg_id) => msg_id,
        Err(_) => {
            msg.channel_id.say(ctx, "Invalid Message").await?;
    ...
    if ctx.http.get_message(channel_id.0, msg_id).await.is_err() {
        msg.channel_id
            .say(ctx, "Invalid Channel and Message")
            .await?;
    ...
    let emoji_string = match args.single::<String>() {
        ...
        Err(_) => {
            msg.channel_id
                .say(ctx, "Please provide a valid emoji")
                .await?;
        ...
        .await
    {
        Ok(_) => {
            ...
            sqlx::query("INSERT INTO reaction_roles VALUES($1, $2, $3, $4, $5, $6, $7)")
                ...
                .bind(reaction_emoji.animated)
                .bind(reaction_emoji.name)
                .fetch_one(&pool)
                .await?;
        }
        Err(e) => {
